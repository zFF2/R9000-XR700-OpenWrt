...
var callGetVersions = rpc.declare({
    object: 'luci.geomate',
    method: 'getVersions',
    params: [],
    expect: { backend_version: 'unknown', frontend_version: 'unknown' }
});
...
return view.extend({
                ...
                if (event.data.type === 'mapReady') {
                    ...
                    self.loadGeoFilters()
                        .then(() => {
                            ...
                            // Static package version display
    ...
    // Show current package versions (updates are handled via apk/opkg)
    updateVersionDisplay: function() {
        ...
        if (statusElement) {
            statusElement.textContent = _('Loading version...');
        ...
        return callGetVersions().then(function(data) {
            ...
            if (versionDisplayElement) {
                versionDisplayElement.textContent =
                    'Backend: ' + formatVersionDisplay(backendVersion) +
                    ' | Frontend: ' + formatVersionDisplay(frontendVersion);
            }
